...
def setup_database():
    ...
    admin_email = 'admin@example.com'.lower()
    ...
    if not user_exists:
        # Insert admin user
        cursor.execute("INSERT INTO users (email, password, role, securityQuestion, securityQuestionAnswer) VALUES (?, ?, ?, ?, ?)", 
                      (admin_email, hashed_password, admin_role, security_question, hashed_security_answer))
        conn.commit()
    ...
    for category in expert_categories:
        expert_email = f'{category}@example.com'.lower()
        ...
        if not user_exists:
            # Insert expert user
            cursor.execute("INSERT INTO users (email, password, role, securityQuestion, securityQuestionAnswer) VALUES (?, ?, ?, ?, ?)", 
                          (expert_email, hashed_password, category, security_question, hashed_security_answer))
            conn.commit()
