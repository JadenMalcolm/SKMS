  ...
  <div class="main-container">
    <div class="ask-container">
      ...
      <div class="search-container">
        <h2>Search Threat Questions</h2>
        <input
          type="text"
          v-model="searchQuery"
          placeholder="Search threat questions..."
          class="search-input"
        />
    ...
    <div class="questions-box">
      <h3>Threat Questions</h3>
      <ul>
        <li v-for="(q, index) in filteredThreatQuestions" :key="index">
...
const threatQuestions = ref<Question[]>([])
const currentUser = ref<User | null>(null)

const filteredThreatQuestions = computed(() => {
  return threatQuestions.value.filter(q => q.question.toLowerCase().includes(searchQuery.value.toLowerCase()))
})
...
onMounted(async () => {
  ...
  try {
    ...
    threatQuestions.value = allQuestions.value.filter(q => q.category === 'Threat')
  } catch (error) {
...
const submitQuestion = async () => {
  if (newQuestionText.value.trim()) {
    try {
      ...
      threatQuestions.value.unshift(newQuestion)
...
const searchQuestions = async () => {
  try {
    ...
    threatQuestions.value = response.data.filter((q: { category: string }) => q.category === 'Threat')
  } catch (error) {
