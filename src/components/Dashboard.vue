        ...
        <div class="questions-box">
          ...
          <ul>
            <li v-for="(q, index) in searchResults" :key="index">
              ...
              <small>{{ new Date(q.timestamp).toLocaleString([], { year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit' }) }}</small>
...
const submitQuestion = async () => {
  if (newQuestionText.value.trim()) {
    try {
      ...
      messageText.value = `Your question was saved: ${newQuestionText.value}` // Show success message
      newQuestionText.value = ''
    } catch (error) {
      console.error('Error submitting question:', error)
      messageText.value = 'Failed to save the question.' // Show error message
    }
  } else {
    messageText.value = 'Please enter a question to ask.' // Show validation message
  }
}
</script>
