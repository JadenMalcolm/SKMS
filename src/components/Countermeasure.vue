  ...
  <div class="main-container">
    <div class="ask-container">
      ...
      <div class="search-container">
        <h2>Search Countermeasure Questions</h2>
        <input
          type="text"
          v-model="searchQuery"
          placeholder="Search countermeasure questions..."
          class="search-input"
        />
    ...
    <div class="questions-box">
      <h3>Countermeasure Questions</h3>
      <ul>
        <li v-for="(q, index) in filteredCountermeasureQuestions" :key="index">
...
const countermeasureQuestions = ref<Question[]>([])
const currentUser = ref<User | null>(null)

const filteredCountermeasureQuestions = computed(() => {
  return countermeasureQuestions.value.filter(q => q.question.toLowerCase().includes(searchQuery.value.toLowerCase()))
})
...
onMounted(async () => {
  ...
  try {
    ...
    countermeasureQuestions.value = allQuestions.value.filter(q => q.category === 'Countermeasure')
  } catch (error) {
...
const submitQuestion = async () => {
  if (newQuestionText.value.trim()) {
    try {
      ...
      countermeasureQuestions.value.unshift(newQuestion)
...
const searchQuestions = async () => {
  try {
    ...
    countermeasureQuestions.value = response.data.filter((q: { category: string }) => q.category === 'Countermeasure')
  } catch (error) {
