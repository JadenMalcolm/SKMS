<template>
  <div class="main-container">
    <div class="ask-container">
      <h2>Ask a Question</h2>
      <input
        type="text"
        v-model="newQuestionText"
        placeholder="Type your question here..."
        class="ask-input"
      />
      <button @click="submitQuestion" class="ask-button">Submit Question</button>
      <div class="search-container">
        <h2>Search Defense Strategy Questions</h2>
        <input
          type="text"
          v-model="searchQuery"
          placeholder="Search defense strategy questions..."
          class="search-input"
        />
        <button @click="searchQuestions" class="search-button">Search</button>
      </div>
    </div>
    <div class="questions-box">
      <h3>Defense Strategy Questions</h3>
      <ul>
        <li v-for="(q, index) in filteredDefenseStrategyQuestions" :key="index">
          <router-link :to="`/question/${q.id}`">{{ q.question }}</router-link>
          <small>{{
            new Date(q.timestamp).toLocaleString([], {
              year: 'numeric',
              month: '2-digit',
              day: '2-digit',
              hour: '2-digit',
              minute: '2-digit',
            })
          }}</small>
          <small>Asked by: {{ q.user_email }}</small>
        </li>
      </ul>
    </div>
  </div>
</template>

<script setup lang="ts">
import { ref, onMounted, computed } from 'vue'
import axios from 'axios'
import { useRouter } from 'vue-router'

interface Question {
  id: number
  question: string
  category: string
  timestamp: string
  user_email: string
  isEditing?: boolean
  editText?: string
  report_count?: number
}

interface User {
  id: number
  email: string
  role: string
}

const router = useRouter()
const newQuestionText = ref('')
const searchQuery = ref('')
const allQuestions = ref<Question[]>([])
const defenseStrategyQuestions = ref<Question[]>([])
const currentUser = ref<User | null>(null)

const filteredDefenseStrategyQuestions = computed(() => {
  return defenseStrategyQuestions.value.filter(q => q.question.toLowerCase().includes(searchQuery.value.toLowerCase()))
})

onMounted(async () => {
  const storedUser = sessionStorage.getItem('user')
  if (storedUser) {
    currentUser.value = JSON.parse(storedUser)
  } else {
    alert('Session expired. Please log in again.')
    router.push('/')
  }

  try {
    const response = await axios.get('http://localhost:5000/questions')
    allQuestions.value = response.data
    defenseStrategyQuestions.value = allQuestions.value.filter(q => q.category === 'Defense Strategy')
  } catch (error) {
    console.error('Error fetching questions:', error)
  }
})

const submitQuestion = async () => {
  if (newQuestionText.value.trim()) {
    try {
      if (!currentUser.value) throw new Error('User not logged in.')

      const response = await axios.post('http://localhost:5000/questions', {
        userId: currentUser.value.id,
        question: newQuestionText.value,
        category: 'Defense Strategy'
      })

      const newQuestion = {
        id: response.data.id,
        question: newQuestionText.value,
        category: 'Defense Strategy',
        timestamp: new Date().toLocaleString(),
        user_email: currentUser.value.email,
        isEditing: false,
        editText: newQuestionText.value,
      }
      allQuestions.value.unshift(newQuestion)
      defenseStrategyQuestions.value.unshift(newQuestion)

      alert(`Your question was saved: ${newQuestionText.value}`)
      newQuestionText.value = ''
    } catch (error) {
      console.error('Error submitting question:', error)
      alert('Failed to save the question.')
    }
  } else {
    alert('Please enter a question to ask.')
  }
}

const searchQuestions = async () => {
  try {
    const response = await axios.post('http://localhost:5000/questions/search', {
      query: searchQuery.value,
    })
    defenseStrategyQuestions.value = response.data.filter((q: { category: string }) => q.category === 'Defense Strategy')
  } catch (error) {
    console.error('Error searching questions:', error)
  }
}
</script>

<style scoped>
.main-container {
  display: flex;
  justify-content: space-between;
  padding: 10px;
  background-color: #f0f0f0;
}
.ask-container {
  width: 40%;
  padding: 15px;
  background-color: #fff;
  border-radius: 10px;
  box-shadow: 0 4px 15px rgba(0, 0, 0, 0.1);
  text-align: center;
  margin-right: 10px;
}
.ask-container h2 {
  font-size: 1rem;
  color: #333;
}
.ask-input,
.search-input {
  width: 98%;
  padding: 8px;
  margin-bottom: 8px;
  border: 1px solid #ccc;
  border-radius: 5px;
}
.ask-button,
.search-button {
  padding: 8px 12px;
  background-color: #4caf50;
  color: white;
  border: none;
  border-radius: 5px;
  cursor: pointer;
  font-size: 14px;
}
.ask-button:hover,
.search-button:hover {
  background-color: #45a049;
}
.questions-box {
  width: 65%;
  padding: 15px;
  background-color: #fff;
  border-radius: 10px;
  box-shadow: 0 4px 15px rgba(0, 0, 0, 0.1);
  padding-right: 20px;
}
.questions-box h3 {
  font-size: 1rem;
  color: #333;
}
.questions-box ul {
  list-style: none;
  padding: 0;
}
.questions-box li {
  margin-bottom: 8px;
  padding: 8px;
  background-color: #f9f9f9;
  border-radius: 5px;
  box-shadow: 0 2px 5px rgba(0, 0, 0, 0.1);
}
.questions-box li small {
  display: block;
  font-size: 0.7rem;
  color: #666;
}
.questions-box li a {
  text-decoration: none;
  color: #007bff;
}
.questions-box li a:hover {
  text-decoration: underline;
}
</style>
